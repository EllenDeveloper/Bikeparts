...
├── lastLogin (Timestamp)
├── preferredPaymentMethod (Enum: KLARNA, PAYPAL, CREDIT_CARD, BANK_TRANSFER)
├── language (String: DE, EN)
├── preferredCurrency (Enum: EUR, USD, CHF)
...
├── quantity (Integer)
├── targetPrice (BigDecimal) - Gewünschter Preis (optional)
├── notes (String)
├── createdAt (Timestamp)
└── updatedAt (Timestamp)
```
...
## 6. SEARCH_RESULT (Gespeicherte Suchergebnisse)
...
## 7. PRODUCT_OFFER (Produkt-Angebot von Shop)

```
PRODUCT_OFFER
├── id (PK, Long)
├── searchResultId (FK, Long) → SEARCH_RESULT.id
├── cartItemId (FK, Long) → CART_ITEM.id
├── productName (String)
├── shopName (String)
├── shopUrl (String)
├── productUrl (String)
...
├── totalPrice (BigDecimal) - Preis + Versand
├── availability (Enum: IN_STOCK, LOW_STOCK, OUT_OF_STOCK, PREORDER)
├── deliveryTime (String) - z.B. "2-3 Werktage"
├── paymentMethods (String) - Komma-getrennt
├── rating (BigDecimal) - Shop/Produkt-Bewertung
├── source (Enum: AMAZON, EBAY, IDEALO, WEB_SCRAPING)
├── dataSource (String) - API oder URL
├── fetchedAt (Timestamp)
└── isValid (Boolean) - Noch aktuell?
```

**Relationen:**
- `N:1` zu SEARCH_RESULT
- `N:1` zu CART_ITEM
